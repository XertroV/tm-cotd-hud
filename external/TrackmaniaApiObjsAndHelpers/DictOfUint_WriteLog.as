class DictOfUint_WriteLog {
  ...
  DictOfUint_WriteLog(const string &in logDir, const string &in logFile) {
    @_d = dictionary();
  ...
  uint GetOr(const string &in key, uint value) const {
    if (Exists(key)) {
      return Get(key);
    }
    return value;
  ...
  _DictOfUint_WriteLog::KvPair@ GetItem(const string &in key) const {
    return _DictOfUint_WriteLog::KvPair(key, Get(key));
  }

  array<_DictOfUint_WriteLog::KvPair@>@ GetItems() const {
    array<_DictOfUint_WriteLog::KvPair@> ret = array<_DictOfUint_WriteLog::KvPair@>(GetSize());
  ...
  private void InitLog(const string &in logDir, const string &in logFile) {
    ...
    log_trace('DictOfUint_WriteLog dir: ' + logDir + ' | logFile: ' + logFile);
    if (logDir.Length == 0) {
  ...
  private void LoadWriteLogFromDisk() {
    if (IO::FileExists(_logPath)) {
      ...
      f.Close();
      while (!fb.AtEnd()) {
        auto kv = _DictOfUint_WriteLog::_KvPair::ReadFromBuffer(fb);
        _d[K(kv.key)] = kv.val;
      }
      log_trace('\\$a4fDictOfUint_WriteLog\\$777 loaded \\$a4f' + GetSize() + '\\$777 entries from: \\$a4f' + _logPath + '\\$777 in \\$a4f' + (Time::Now - start) + ' ms\\$777.');
      f.Close();
    } else {
  ...
  private void WriteOnSet(const string &in key, uint value) {
    _DictOfUint_WriteLog::KvPair@ p = _DictOfUint_WriteLog::KvPair(key, value);
    Buffer@ buf = Buffer();
    p.WriteToBuffer(buf);
    buf.Seek(0, 0);
...
namespace _DictOfUint_WriteLog {
