class DictOfTrackOfTheDayEntry_WriteLog {
  ...
  TrackOfTheDayEntry@ GetOr(const string &in key, TrackOfTheDayEntry@ value) const {
    if (Exists(key)) {
      return Get(key);
    }
    return value;
  ...
  private void InitLog(const string &in logDir, const string &in logFile) {
    ...
    log_trace('DictOfTrackOfTheDayEntry_WriteLog dir: ' + logDir + ' | logFile: ' + logFile);
    if (logDir.Length == 0) {
  ...
  private void LoadWriteLogFromDisk() {
    if (IO::FileExists(_logPath)) {
      ...
      log_trace('\\$a4fDictOfTrackOfTheDayEntry_WriteLog\\$777 loaded \\$a4f' + GetSize() + '\\$777 entries from: \\$a4f' + _logPath + '\\$777 in \\$a4f' + (Time::Now - start) + ' ms\\$777.');
      f.Close();
    } else {
...
namespace _DictOfTrackOfTheDayEntry_WriteLog {
  ...
  namespace _KvPair {
    ...
    KvPair@ ReadFromBuffer(Buffer@ &in buf) {
      ...
      /* Parse field: val of type: TrackOfTheDayEntry@ */
      TrackOfTheDayEntry@ val = _TrackOfTheDayEntry::ReadFromBuffer(buf);
      return KvPair(key, val);
    }
