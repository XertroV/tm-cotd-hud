shared class DictOfInt_WriteLog {
  ...
  private void InitLog(const string &in logDir, const string &in logFile) {
    ...
    trace('DictOfInt_WriteLog dir: ' + logDir + ' | logFile: ' + logFile);
    if (logDir.Length == 0) {
  ...
  private void LoadWriteLogFromDisk() {
    if (IO::FileExists(_logPath)) {
      ...
      trace('\\$a4fDictOfInt_WriteLog\\$777 loaded \\$a4f' + GetSize() + '\\$777 entries from: \\$a4f' + _logPath + '\\$777 in \\$a4f' + (Time::Now - start) + ' ms\\$777.');
      f.Close();
    } else {
