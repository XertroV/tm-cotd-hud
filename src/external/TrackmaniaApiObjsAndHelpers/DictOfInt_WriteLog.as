class DictOfInt_WriteLog {
  ...
  DictOfInt_WriteLog(const string &in logDir, const string &in logFile) {
    @_d = dictionary();
  ...
  int GetOr(const string &in key, int value) const {
    if (Exists(key)) {
      return Get(key);
    }
    return value;
  ...
  _DictOfInt_WriteLog::KvPair@ GetItem(const string &in key) const {
    return _DictOfInt_WriteLog::KvPair(key, Get(key));
  }

  array<_DictOfInt_WriteLog::KvPair@>@ GetItems() const {
    array<_DictOfInt_WriteLog::KvPair@> ret = array<_DictOfInt_WriteLog::KvPair@>(GetSize());
  ...
  private void InitLog(const string &in logDir, const string &in logFile) {
    ...
    log_trace('DictOfInt_WriteLog dir: ' + logDir + ' | logFile: ' + logFile);
    if (logDir.Length == 0) {
  ...
  private void LoadWriteLogFromDisk() {
    if (IO::FileExists(_logPath)) {
      ...
      f.Close();
      while (!fb.AtEnd()) {
        auto kv = _DictOfInt_WriteLog::_KvPair::ReadFromBuffer(fb);
        _d[K(kv.key)] = kv.val;
      }
      log_trace('\\$a4fDictOfInt_WriteLog\\$777 loaded \\$a4f' + GetSize() + '\\$777 entries from: \\$a4f' + _logPath + '\\$777 in \\$a4f' + (Time::Now - start) + ' ms\\$777.');
      f.Close();
    } else {
  ...
  private void WriteOnSet(const string &in key, int value) {
    _DictOfInt_WriteLog::KvPair@ p = _DictOfInt_WriteLog::KvPair(key, value);
    Buffer@ buf = Buffer();
    p.WriteToBuffer(buf);
    buf.Seek(0, 0);
