class DictOfBool_WriteLog {
  ...
  DictOfBool_WriteLog(const string &in logDir, const string &in logFile) {
    @_d = dictionary();
  ...
  bool GetOr(const string &in key, bool value) const {
    if (Exists(key)) {
      return Get(key);
    }
    return value;
  ...
  _DictOfBool_WriteLog::KvPair@ GetItem(const string &in key) const {
    return _DictOfBool_WriteLog::KvPair(key, Get(key));
  }

  array<_DictOfBool_WriteLog::KvPair@>@ GetItems() const {
    array<_DictOfBool_WriteLog::KvPair@> ret = array<_DictOfBool_WriteLog::KvPair@>(GetSize());
  ...
  private void InitLog(const string &in logDir, const string &in logFile) {
    ...
    log_trace('DictOfBool_WriteLog dir: ' + logDir + ' | logFile: ' + logFile);
    if (logDir.Length == 0) {
  ...
  private void LoadWriteLogFromDisk() {
    if (IO::FileExists(_logPath)) {
      ...
      log_trace('\\$a4fDictOfBool_WriteLog\\$777 loaded \\$a4f' + GetSize() + '\\$777 entries from: \\$a4f' + _logPath + '\\$777 in \\$a4f' + (Time::Now - start) + ' ms\\$777.');
      f.Close();
    } else {
...
namespace _DictOfBool_WriteLog {
