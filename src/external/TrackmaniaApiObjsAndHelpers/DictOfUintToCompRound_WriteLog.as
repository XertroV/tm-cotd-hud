class DictOfUintToCompRound_WriteLog {
  ...
  CompRound@ GetOr(uint key, CompRound@ value) const {
    if (Exists(key)) {
      return Get(key);
    }
    return value;
  ...
  private void InitLog(const string &in logDir, const string &in logFile) {
    ...
    log_trace('DictOfUintToCompRound_WriteLog dir: ' + logDir + ' | logFile: ' + logFile);
    if (logDir.Length == 0) {
  ...
  private void LoadWriteLogFromDisk() {
    if (IO::FileExists(_logPath)) {
      ...
      log_trace('\\$a4fDictOfUintToCompRound_WriteLog\\$777 loaded \\$a4f' + GetSize() + '\\$777 entries from: \\$a4f' + _logPath + '\\$777 in \\$a4f' + (Time::Now - start) + ' ms\\$777.');
      f.Close();
    } else {
